...
<script type="text/javascript">
	var QueryUrl="http://localhost:8080/<%=request.getContextPath()%>/user/queryAll";
	$(function() {	
		//1.初始化Table
					                        ...
					                        $.ajax({
					                            ...
					                            success: function (data, status) {
					                                $.each(JSON.parse(data), function (key, value) {
					                                	if(value.rname!="超级管理员"){
					                                		result.push({ value: value.rname, text: value.rname });
					                                	}
					                                });
					                            }
					                        });
	...
	$().ready(function() {
		$("#btn_delete").click(function() {
			...
			if (rows.length > 0 && cj==true) {
				var r = confirm("您确定要删除吗,删除后该用户将无法再登录本系统")
				if (r) {
...
<body>
	<div class="panel-body" style="padding-bottom: 0px;">
		...
		<div id="toolbar" class="btn-group">
			...
			<font style="color:red;	font-size: 11px;">注:仅可对超级管理员以外的角色执行删除&修改操作</font>
		</div>
