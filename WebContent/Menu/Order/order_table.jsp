...
<script type="text/javascript">
	var QueryUrl="http://localhost:8080/<%=request.getContextPath()%>/order/queryAll";
	$(function() {	
		//1.初始化Table
								...
								columns : [
										...
										},
										{
											...
											formatter : function(value, row,
												...
												var strHtml='';
												if(row.status=='待发货'){
													strHtml = '<button onclick="OrderSend('+
															row.orderid+')">发货</button>';
												}else if(row.status=='退款中'){
													strHtml='<button onclick="OrderComplite('+
													row.orderid+')">退款</button>';
												}else{
													strHtml='无操作';
												}
												return strHtml;
	...
	function OrderComplite(orderid) {
        $.ajax({
            type: 'GET',
            url: "complite",
            data: {"orderid":orderid},
